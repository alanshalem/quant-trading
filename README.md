...
│   ├── 02_classical_strategies/        # Momentum + mean reversion (pandas, no ML)
│   ├── 03_ml_strategy/                 # 3-part ML strategy (vectorized paradigm)
│   └── 04_event_driven_strategies/     # Envelope + SMA backtests (event-driven paradigm)
│
...
### Module 2: Classical Strategies (`02_classical_strategies/`)

Rule-based, vectorized strategies with **pandas only** — no ML, no library.
Both derive an edge from `sign(previous log return)` and backtest it end to end
with round-trip fees. Data ships as committed CSVs in `data/samples/` (runs
offline).
...
### Module 4: Event-Driven Strategies (`04_event_driven_strategies/`)
